...
void printList(FILE *fp){
	...
	fprintf(fp, "Total memory allocated: %d", listCounter);
}
...
struct mB* search(int check, struct mB ** prev, bool list){
	...
	if(list){
		...
		*prev = temp;
		...
		count = test->size;
		while(listCheck){
			if(((test->size - check) < (bestFit - check)) && (test->size - check) >= 0){
				fitHolder = test;
				if(temp != NULL){
					prevHolder = temp;
				}
				bestFit = fitHolder->size;
				if(fitHolder->size == check){
					break;
				}
			}
			if(test->next == NULL){
...
int bestFit(int check, char * data){
	struct mB *prev = NULL;
	struct mB *del = NULL;
...
int main(int argc, char *argv[])
{
	...
	char * extraFile = ".txt";
	char * outputFile = "BestFitFirstN";
	char * outputFile2 = "BestFitMiddle";
	if(argc != 2){
	...
	//Beginning of the best fit algorithm
	limit = FIRSTADD;
	i = 0;
	while(fgets(str, MAXCHAR, fp) != NULL){
		token = strtok(str, "\n");
	...
	printf("The Best Fit algorithm, by Rhys Mallia S3656436 :)\n");
	return 0;
}